...
func InsertProduct(body string, User string) (int, string) {
	...
	if err2 != nil {
		return 400, "Ocurrió un error al intentar realizar el registro de producto" + t.ProdTitle + " > " + err2.Error()
	}
...
func SelectProduct(request events.APIGatewayV2HTTPRequest) (int, string) {
	...
	orderField = param["orderField"] // 'I' product_Id, 'T' title, 'D' Description, 'F' Created At
	...
	if err3 != nil {
		return 400, "Ocurrió un error al intentar convertir JSON la busqueda de Productos"
	}
...
func UpdateStock(body string, User string, id int) (int, string) {
	...
	if err2 != nil {
		return 400, "Ocurrio un error al intentar realizar UPDATE del stock " + strconv.Itoa(id) + " > " + err2.Error()
	}
